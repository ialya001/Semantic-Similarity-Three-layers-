...
class OptimalAlignment:
    def __init__(self, seq1: Iterable, seq2: Iterable):
        ...
        self.smatrix = ScoreMatrix(match=1, miss=-1, partial_match=0.9,lemmaPOS=0.8,gap=-1)
        self._score = int()
    ...
    def change_matrix(self, newmatrix):
        ...
        Args:
            newmatrix (ScoreMatrix): Matrix containing match, miss, partial_match, lemmaPOS, and gap penalties.
        """
    ...
    def _fill_matrices(self):
        def are_synonyms(x,y):# To check for synonyms between seq1 and seq2 return (True or False) 
             ...
             if x in synonyms:
                  ...
                  return False
        def get_wordnet_pos(word):#To get the lemmatization and POS of a word
             tag = nltk.pos_tag([word])[0][1][0].upper()
             tag_dict = {"J": wordnet.ADJ,"N": wordnet.NOUN,"V": wordnet.VERB,"R": wordnet.ADV}
             return tag_dict.get(tag, wordnet.NOUN)

        def checklemma_pos(word1,word2):#Compare the similarity of lemmatize and POS form(with the help of the above function(def get_wordnet_pos)
			# 1. Init Lemmatizer
        ...
        for irow in range(0, len(self.seq2)):
            for jcol in range(0, len(self.seq1)):
                ...
                elif are_synonyms(self.seq1[jcol], self.seq2[irow]):
                    diagscore += self.smatrix.partial_match
                elif checklemma_pos(self.seq1[jcol], self.seq2[irow]):
                    diagscore += self.smatrix.lemmaPOS  
        ...
        while True:
            if self._pmatrix[irow][jcol] == "diag":
                ...
                if self.seq1[jcol - 1] == self.seq2[irow - 1]:
                   self._identity += 1
                elif are_synonyms(self.seq1[jcol-1], self.seq2[irow-1]):
                   self._identity += 0.9
                elif checklemma_pos(self.seq1[jcol-1], self.seq2[irow-1]):
                   self._identity += 0.8
...
class ScoreMatrix(object):
    def __init__(self, match: int, miss: int, partial_match: float,lemmaPOS: float, gap: int):
        self.match = match
        self.miss = miss
        ...
        try:
            ...
            assert isinstance(self.partial_match, float) is True
            assert isinstance(self.lemmaPOS, float) is True
            assert isinstance(self.gap, int) is True
        except AssertionError:
            raise ValueError("match, miss, and gap should be integers.Partial_match should be float")

    def __str__(self):
        return f"Match:{self.match} Missmatch:{self.miss} PartialMatch:{self.partial_match} Lemma&POS:{self.lemmaPOS} Gap:{self.gap}"
